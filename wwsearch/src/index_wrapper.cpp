...
SearchStatus DefaultIndexWrapper::Open(bool use_rocksdb, bool use_compression) {
  ...
  // index config
  {
    // TokenizerMMSEG requires dictionary assets (e.g. `wwsearch_mmseg_uni.lib`).
    // Some lightweight environments ship only code without dict files; in that case
    // fall back to a space-splitting tokenizer so the library remains runnable.
    const std::string dict_dir =
        params_.mmseg_dict_dir_.size() == 0 ? "./" : params_.mmseg_dict_dir_;
    const std::string uni_lib = dict_dir + "/wwsearch_mmseg_uni.lib";
    FILE *fp = fopen(uni_lib.c_str(), "rb");
    if (fp != nullptr) {
      fclose(fp);
      ...
      SearchLogInfo("mmseg dict not found (%s), fallback to TokenizerSpaceImpl",
                    uni_lib.c_str());
      tokenizer_ = new TokenizerSpaceImpl();
    }
  }
